...
description: >
  ...
  services, check status, configure settings, update, backup, troubleshoot, or
  manage the Forge repo index. Covers all CLI commands, common workflows, and
  Docker/Podman differences.
---

# Horus CLI — Command Reference

The `horus` CLI manages the Horus Docker/Podman Compose stack (Anvil, Vault, Forge, Typesense). It handles setup, lifecycle, configuration, diagnostics, and repository index management.
...
| `horus repo` | Manage the Forge repository index |
...
Manage the Forge repository index.

```
horus repo rindex    # Trigger a full repository index rescan
horus repo scan      # Alias for rindex
```

### `horus repo rindex`

Triggers a full rescan of all configured Forge scan paths via the `forge_repo_scan` MCP tool over HTTP. Use this after cloning a new repository to make it discoverable by Forge agents without restarting the stack.
...
✓ Repository scan complete
  ...
  ✓ MyNewRepo     /Users/you/Desktop/Repositories/MyNewRepo
  ✓ Horus         /Users/you/Desktop/Repositories/ArjunKhera/Horus
  ...
```

Requires Horus to be running (`horus up`). Calls Forge at the configured port (default: 8200).
...
```bash
git clone git@github.com:org/my-new-repo.git ~/Desktop/Repositories/my-new-repo
horus repo rindex
# Now forge_repo_resolve and forge_develop will find it
```
