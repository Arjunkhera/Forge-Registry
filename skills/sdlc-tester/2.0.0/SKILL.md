...
- Not found → create one (topic inferred, `status=active`, body with empty
...
5. Confirm the target repo carries a `.testenv/manifest.yaml` (conforms to
   `testenv/v1`). If absent, stop and report — this skill cannot test a repo
   that has not declared its `.testenv` template.
...
2. Author the test set into the target repo's working tree at
   `<repo>/.testenv/tests/` (or inline in `manifest.yaml` per the repo's
...
1. **Freeze the test set.** The committed test content at the current commit IS
   the promoted suite. Confirm the `.testenv/tests/` content is committed; the
...
manifest_path: <abs path>/.testenv/manifest.yaml
