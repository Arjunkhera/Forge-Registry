...
Per design `cae9fb8d` decision **E2 / A3**: the test spec is generated from the acceptance criteria and any `#deviation` journal entries **before** implementation. Do this immediately after the code session exists and before writing any production code.

1. **Invoke the `sdlc-tester` skill** (v2.x rewritten orchestration skill) against this work item in **spec-generation / RED mode**. Pass the work item ID, the active `sessionPath`, and the acceptance criteria. Tester authors the do/check/proof test set into the in-repo `.testenv/tests/`, spawns the `sdlc-testenv` executor, and produces a **RED test-run** (tests must fail because the feature does not yet exist).
...
4. Run linter if available (check Vault repo profile for `lint` command)
...
1. **Invoke the `sdlc-tester` skill at the green gate.** Pass the work item ID and the active `sessionPath`. Tester freezes the in-repo test set, spawns the `sdlc-testenv` executor for a final run, and on success performs the atomic promotion: bind the proof-of-work report to the committed test content (commit/hash), generate the read-only Anvil `test-spec` + `test-run` mirror, and set the `story.proof_of_work` edge to the GREEN test-run.
