...
description: >
  ...
  manage workspaces, discover repos, trigger a repo rescan (forge_repo_scan),
  resolve git workflows, publish artifacts, or install skills and plugins.
  Covers all 17 Forge tools, registry architecture, 5 artifact types,
  workspace inheritance, and multi-agent sessions.
---
...
```
forge_publish(ref: "skill:horus-forge@2.1.0", registry: "local")
forge_publish(ref: "skill:horus-forge@2.1.0", registry: "forge-registry")
```
...
Returns: name, local path, remote URL, default branch, language, framework. If the repo isn't found, call `forge_repo_scan` to refresh the index and retry.

### On-demand index rescan

```
forge_repo_scan()
```

Triggers a full rescan of all configured scan paths. Use this after cloning a new repository to register it without restarting Horus. Returns `{ scanPaths, reposFound, repos[] }`. Also available as `horus repo rindex` from the CLI.
...
| Using docker exec to trigger a rescan | Call `forge_repo_scan` (MCP) or `horus repo rindex` (CLI) instead. |
