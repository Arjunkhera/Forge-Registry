...
2. **Check for an existing PR** by running `gh pr list --head {branch-name}` — if the PR was already created, re-add the PR link to the work item via `anvil_update_note` (do not create a duplicate PR).
3. **Re-apply status transition** (`in_review`) if the work item is still in a prior state via `anvil_update_note`.
4. **Confirm with the user** what was recovered before continuing.
