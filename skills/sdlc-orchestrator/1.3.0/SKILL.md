...
description: >
  ...
  story, planner, designer, developer, tester, reviewer, docs, sdlc-bug, sdlc-spec,
  sdlc-feature-doc, sdlc-keystone-update), and maintains cross-cutting concerns like board views
  and program aggregation.
---
...
4. `anvil_search` for `bug` type entities with status `open` or `investigating` older than 14 days → flag for triage (either assign root cause or close as wont-fix)
5. `forge_workspace_list` for stale workspaces → suggest clean
...
3. Generate changelog grouped by type/subtype: Features (story/feature), Bug Fixes (story/bugfix + verified bugs), Refactors, Chores
...
7. Trigger documentation sweep (→ docs skill)
8. Log release in project scratch via `anvil_create_entity` (journal type)
...
| "Report a bug / found a bug / log a bug in X" | **sdlc-bug** skill → report |
| "Write requirements for X / create a spec for X" | **sdlc-spec** skill → create |
| "Create a design doc for X / start a feature doc" | **sdlc-feature-doc** skill → create |
| "Update the keystone for X / reformat the project page" | **sdlc-keystone-update** skill |
