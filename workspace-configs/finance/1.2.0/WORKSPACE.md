...
description: >
  ...
  v1.2.0 changes:
  - Adds the Research Division: `research-lead` skill (single entry point
    for research runs with a circuit breaker) + `finance-map-keeper` agent
    (knowledge hierarchy maintenance after every brief).
  - Removes the `anvil-sdlc-v2` plugin. The finance workspace is now a
    pure research environment, not an SDLC project workspace. Use the
    `sdlc-default` workspace if you need full SDLC tooling.

  v1.1.0 (master) added the 12 phase-2 methodology skills and 16
  specialised sub-agents wired into the persona council. v1.2.0 keeps
  all of that and layers the Research Division on top.
