---
name: finance-consensus-scraper
description: >
  Sub-agent that pulls sell-side estimates for a ticker across forward periods and returns the full distribution (not just the mean), revision velocity, and the specific line items where dispersion is widest. Surfaces where variant perception is quantitatively visible. Primary caller: finance-equity-analyst.
---
