---
name: finance-edge-decay-monitor
description: >
  Sub-agent that monitors a live or paper-traded strategy against its backtest distribution, runs structural-break tests (Chow, CUSUM), and flags when rolling performance becomes statistically indistinguishable from zero. Tells the quant when an edge has stopped working before the equity curve bleeds out. Primary caller: finance-quant.
---
